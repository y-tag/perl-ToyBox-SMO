...
sub take_step {
    ...
    if ($y1 == $y2) {
        $L = ($alpha2 + $alpha1 - $C > 0) ? $alpha2 + $alpha1 - $C : 0;
        $H = ($alpha2 + $alpha1 < $C)     ? $alpha2 + $alpha1 : $C;
    } else {
        $L = ($alpha2 - $alpha1 > 0)       ? $alpha2 - $alpha1 : 0;
        $H = ($C + $alpha2 - $alpha1 < $C) ? $C + $alpha2 - $alpha1 : $C;
    }
    ...
    $self->update_weight($datum1, $datum2, $a1, $a2);
...
sub update_weight {
    my ($self, $datum1, $datum2, $a1, $a2) = @_;

    my $tmp1 = $datum1->{y} * ($a1 - $datum1->{alpha});
    my $tmp2 = $datum2->{y} * ($a2 - $datum2->{alpha});

    while (my ($f, $v) = each %{$datum1->{x}}) {
        $self->{w}{$f} += $tmp1 * $v;
    }
    while (my ($f, $v) = each %{$datum2->{x}}) {
        $self->{w}{$f} += $tmp2 * $v;
    }
...
ToyBox::LinearSMO - Support Vector Machine with Sequential Minimal Optimization
